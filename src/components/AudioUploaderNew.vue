<template>
    <div class="audio-uploader">
        <el-button size="small" type="primary" id="btn-audio-uploader" v-html="audioUrl ? '更改音频' : '上传音频'"></el-button>
        <h3 class="h3"><strong>{{audioUrl ? waitingTip : ''}}</strong></h3>
        <div class="url">{{audioUrl ? audioUrl.split('/').pop() : ''}}</div>
        ...
        init: {
          ...
          Error: function (up, err) {
            if (err.code == -600) {
              ...
              that.$message.error(`上传的音频大小不能超过 ${unit}!`);
            }
            else if (err.code == -601) {
              that.$message.error(`上传的音频只能是 ${this.allowType.join(',')} 格式!`);
            }
            else {
